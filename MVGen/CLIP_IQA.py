...
def calculate_clip_iqa(scene_folder):
    # 加载CLIP模型和预训练的权重
    ...
    scene_folder = os.path.join(scene_folder, "images")
    # 遍历文件夹中的所有图像
    for filename in os.listdir(scene_folder):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            image_path = os.path.join(scene_folder, filename)
...
def main(input_folder, output_folder):
    ...
    for scene in os.listdir(input_folder):
        scene_folder = os.path.join(input_folder, scene)
