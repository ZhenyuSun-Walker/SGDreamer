...
if args.text_path is not None:
    ...
    # Extend or trim the prompts list to match num
    if len(prompt) < num:
        prompt = (prompt * (num // len(prompt) + 1))[:num]
    elif len(prompt) > num:
        prompt = prompt[:num]
    args.text = prompt[0]
else:
    prompt = [args.text] * num
...
generate_video(image_paths, res_dir, args.fov, args.deg, args.gen_video)
